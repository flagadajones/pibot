            ...
            for subdirname in dirnames:
                ...
                for filename in os.listdir(subject_path):
                    try:
                        ...
                        self.trainset_images.append(
                            numpy.asarray(im, dtype=numpy.uint8))
                        # Et on dit que cet array correspond à l'identité n°c
                        self.trainset_index.append(c)
                    except IOError as erro:
                        logging.error(
                            "I/O error({0}): {1}".format(erro[0], erro[1]))
                    except:
...
class OpenCVFaceRecognitionLBPH(OpenCVGenericRecognition):
    ...
    def train(self):
        ...
        self.model = cv2.face.LBPHFaceRecognizer_create(1, 8, 8, 8, 100)
        #self.model = cv2.face.LBPHFaceRecognizer_create()
        #self.model = cv2.createLBPHFaceRecognizer(radius = 1, grid_x = 6, grid_y = 6)
        self.model.train(numpy.asarray(self.trainset_images),
                         numpy.asarray(self.trainset_index))
...
if __name__ == "__main__":
    ...
    R = OpenCVFaceRecognitionLBPH("./faces/",
