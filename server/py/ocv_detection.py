...
TRAINSET_FACE_FRONTAL = dir_path+"/haarcascades/haarcascade_frontalface_default.xml"
TRAINSET_FACE_PROFILE = dir_path+"/haarcascades/haarcascade_profileface.xml"
TRAINSET_BODY_FULL = dir_path+"/haarcascades/haarcascade_fullbody.xml"
...
class OpenCVGenericDetection:
    ...
    def __init__(self, archive_folder="/tmp/", debug=False):
        """ init
    ...
    def set_frame(self, image_path):
        logging.debug("Image : {0}".format(image_path))
       ...
       # On charge l'image dans une frame
        ...
        self.frame = cv2.cvtColor(image_path,  cv2.COLOR_BGR2GRAY)
        #self.frame = image_path
        logging.debug("Résolution de l'image : '{0}x{1}'".format(
        ...
        if ratio != 1:
            ...
            logging.debug(
                "Redimensionnement de l'image en : {0}".format(newsize))
    ...
    def find_items(self):
        ...
        """
        logging.debug("Recherche des items...")
        # Afin de grouper les archives, nous allons utiliser un préfixe unique
        self.images_prefix = "{0}_{1}_".format(
            datetime.datetime.now().strftime("%Y%m%d-%H%M%S-%f"), self.__class__.__name__)

        # On applique le classifier pour détecter les visages
        items = self.classifier.detectMultiScale(self.frame, 1.3, 5)
        # items = self.classifier.detectMultiScale(
        #    self.frame, scaleFactor=DOWNSCALE, minNeighbors=6, minSize=(40, 40), maxSize=(150, 150))

        # On valorise self.items et on affiche un peu de log
        logging.debug("Nombre d'items : '{0}'".format(len(items)))
        logging.debug("Items = {0}".format(items))
        self.items = items
    ...
    def extract_items_frames(self):
        ...
        """
        logging.debug("Extractions des frames des items ('{0}' a extraire)...".format(
            len(self.items)))
        items_frames = []
    ...
    def get_items_frames(self, grayscale=False):
        ...
        for item_frame in self.items_frames:
            item_frame["frame"] = cv2.cvtColor(
                item_frame["frame"], cv2.COLOR_BGR2GRAY)
    ...
    def archive_items_frames(self):
        ...
        """
        logging.debug("Archive les items ('{0}' à archiver)...".format(
            len(self.items_frames)))
        idx = 0
        ...
        for item_frame in self.items_frames:
            ...
            logging.debug(
                "Archive un item dans le fichier : '{0}'".format(image_name))
            cv2.imwrite(os.path.join(
                self.archive_folder,  image_name), a_frame)
            idx += 1
    ...
    def archive_with_items(self):
        ...
        """
        logging.debug("Archive l'image avec les items trouvés...")
        ...
        logging.debug("Archive file is : '{0}'".format(archive_full_name))
        logging.debug(os.path.join(self.archive_folder,
                                   archive_full_name))
        logging.debug(cv2.imwrite(os.path.join(self.archive_folder,
                                               archive_full_name), self.frame))
